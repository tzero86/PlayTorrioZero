...
class _PlayerScreenState extends State<PlayerScreen>
  ...
  Future<void> _saveTextSyncedCues(
    ...
    if (_currentSubtitlePath != null) {
      ...
      if (oldPath != null && oldPath != newPath) SubtitleService().deleteSubtitleFile(oldPath);
      _setSubtitleScale(_subtitleScale);
  ...
  Widget _buildBackgroundStack() {
    return Stack(
      children: [
        ...
        if (_hasLoadedMedia)
          Positioned.fill(
            // The player texture is the most expensive layer on screen; without
            // this boundary every control fade, volume HUD and subtitle tweak
            // repaints the video surface along with them.
        ...
        // Loading overlay (rendered on top of video to avoid surface recreation)
        if (_isLoading)
          Positioned.fill(
            child: Center(
              child: Column(
