...
>
> **Outcome (recorded after execution): none of these were taken.** The owner named the product **ZPlay**, which shipped as `tzero86/ZPlay`. The §8 ranking in `../RENAME_PRODUCT_PLAN.md` (`Zylova` → `Zeruna` → `ZeroPlay`) is therefore superseded by that decision and is retained only as the historical evaluation — do not treat it as a live recommendation. This annex remains valid for everything else: requirements R1-R10, the candidate table, the surface-form renderings, the do-not-use list, and the cheap-vs-expensive split (§5) — the latter is the operational core of the plan.
>
