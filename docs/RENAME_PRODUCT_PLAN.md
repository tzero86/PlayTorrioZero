...
| Inherited **third-party credentials**: TMDB, Wyzie, Audionest (x2), Paper2Audio, Films365, VidGod, plus upstream-run proxy hosts `db.speedracelight.com` / `api.speedracelight.com` | `tmdb_helper.dart`; `videasy.dart`; `audiobook_scraper_service.dart`; `wyzie_provider.dart` | **Partly done.** TMDb is BYOK-first (`tmdb_service.dart`); the six service credentials now live in one registry (`legacy_upstream_credentials.dart`) with user and build-time overrides exposed in Settings; the two `speedracelight` hosts and the VidGod token are **off by default**. Still running on borrowed third-party values: Audionest (two), Paper2Audio and Films365, each switchable and documented in `docs/CUSTOM_CREDENTIALS.md`. Unauthorised credential reuse risks revocation and a terms breach, independently of GPL. Measured, not inferred. |
...
- Own the Wyzie key: free at `store.wyzie.io/redeem`, paste it in Settings > Service API Keys. The `*.speedracelight.com` hosts and the VidGod token are defaulted off; the Audionest, Paper2Audio and Films365 values are still borrowed third-party credentials and should be replaced or blanked, which costs one audiobook source, the PDF-to-audio generator and two scrapers. See `docs/CUSTOM_CREDENTIALS.md`.
